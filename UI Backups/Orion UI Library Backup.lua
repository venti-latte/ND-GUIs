...
local function GetIcon(IconName)
	if Icons[IconName] ~= nil then return Icons[IconName]
	else return nil end
end
...
function OrionLibraryrary:IsRunning()
	if gethui then return Orion.Parent == gethui()
	else return Orion.Parent == game:GetService"CoreGui" end
end

local function AddConnection(Signal, Function)
	if not OrionLibraryrary:IsRunning() then return end
	local SignalConnect = Signal:Connect(Function)
	table.insert(OrionLibraryrary.Connections, SignalConnect)
	return SignalConnect
end

task.spawn(function()
	while OrionLibraryrary:IsRunning() do wait() end
	for _, Connection in next, OrionLibraryrary.Connections do Connection:Disconnect() end
end)
...
local function CreateElement(ElementName, ElementFunction)
	OrionLibraryrary.Elements[ElementName] = function(...) return ElementFunction(...) end
end

local function MakeElement(ElementName, ...)
	local NewElement = OrionLibraryrary.Elements[ElementName](...)
	return NewElement
end
...
local function AddThemeObject(Object, Type)
	if not OrionLibraryrary.ThemeObjects[Type] then OrionLibraryrary.ThemeObjects[Type] = {} end
	table.insert(OrionLibraryrary.ThemeObjects[Type], Object)
	Object[ReturnProperty(Object)] = OrionLibraryrary.Themes[OrionLibraryrary.SelectedTheme][Type]
	return Object
end

local function SetTheme()
	for Name, Type in pairs(OrionLibraryrary.ThemeObjects) do
		for _, Object in pairs(Type) do
			Object[ReturnProperty(Object)] = OrionLibraryrary.Themes[OrionLibraryrary.SelectedTheme][Name]
		end
	end
end
...
local function LoadCfg(Config)
	...
	table.foreach(Data, function(a, b)
		if OrionLibraryrary.Flags[a] then
			spawn(function()
				if OrionLibraryrary.Flags[a].Type == "Colorpicker" then OrionLibraryrary.Flags[a]:Set(UnpackColor(b))
				else OrionLibraryrary.Flags[a]:Set(b) end
			end)
		else warn("Orion Library Config Loader - Could not find ", a, b) end
	end)
end

local function SaveCfg(Name)
	local Data = {}
	for i, v in pairs(OrionLibraryrary.Flags) do
		if v.Save then
			if v.Type == "Colorpicker" then Data[i] = PackColor(v.Value)
			else Data[i] = v.Value end
		end
	end
	writefile(OrionLibraryrary.Folder .. "/" .. Name .. ".txt", tostring(HttpService:JSONEncode(Data)))
end
...
function OrionLibraryrary:MakeNotification(NotificationConfig)
	spawn(function()
...
function OrionLibraryrary:Init()
	if OrionLibraryrary.SaveCfg then
		pcall(function()
			if isfile(OrionLibraryrary.Folder .. "/" .. game.GameId .. ".txt") then
				LoadCfg(readfile(OrionLibraryrary.Folder .. "/" .. game.GameId .. ".txt"))
...
function OrionLibraryrary:MakeWindow(WindowConfig)
	local FirstTab = true
	...
	OrionLibraryrary.Folder = WindowConfig.ConfigFolder
	OrionLibraryrary.SaveCfg = WindowConfig.SaveConfig
	...
	AddConnection(CloseBtn.MouseButton1Up, function()
		...
		UIHidden = true
		OrionLibraryrary:MakeNotification{ Name = "Interface Hidden", Content = "Tap RightShift to reopen the interface", Time = 5 }
	...
	function TabFunction:MakeTab(TabConfig)
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddButton(ButtonConfig)
				...
				AddConnection(Click.MouseEnter, function()
					TweenService:Create(ButtonFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
						BackgroundColor3 = Color3.fromRGB(OrionLibraryrary.Themes[OrionLibraryrary.SelectedTheme].Second.R * 255 + 3, OrionLibraryrary.Themes[OrionLibraryrary.SelectedTheme].Second.G * 255 + 3, OrionLibraryrary.Themes[OrionLibraryrary.SelectedTheme].Second.B * 255 + 3)
					}):Play()
				end)
				AddConnection(Click.MouseLeave, function()
					TweenService:Create(ButtonFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
						BackgroundColor3 = OrionLibraryrary.Themes[OrionLibraryrary.SelectedTheme].Second
					}):Play()
				end)
				...
				AddConnection(Click.MouseButton1Down, function()
					TweenService:Create(ButtonFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
						BackgroundColor3 = Color3.fromRGB(OrionLibraryrary.Themes[OrionLibraryrary.SelectedTheme].Second.R * 255 + 6, OrionLibraryrary.Themes[OrionLibraryrary.SelectedTheme].Second.G * 255 + 6, OrionLibraryrary.Themes[OrionLibraryrary.SelectedTheme].Second.B * 255 + 6)
					}):Play()
				end)
			...
			function ElementFunction:AddToggle(ToggleConfig)
				...
				function Toggle:Set(Value)
					...
					TweenService:Create(ToggleBox, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
						BackgroundColor3 = Toggle.Value and ToggleConfig.Color or OrionLibraryrary.Themes.Default.Divider
					}):Play()
					TweenService:Create(ToggleBox.Stroke, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
						Color = Toggle.Value and ToggleConfig.Color or OrionLibraryrary.Themes.Default.Stroke
					}):Play()
				...
				if ToggleConfig.Flag then OrionLibraryrary.Flags[ToggleConfig.Flag] = Toggle end
				return Toggle
			end
			...
			function ElementFunction:AddSlider(SliderConfig)
				...
				if SliderConfig.Flag then OrionLibraryrary.Flags[SliderConfig.Flag] = Slider end
				return Slider
			end
			...
			function ElementFunction:AddDropdown(DropdownConfig)
				...
				if DropdownConfig.Flag then OrionLibraryrary.Flags[DropdownConfig.Flag] = Dropdown end
				return Dropdown
			end
			...
			function ElementFunction:AddBind(BindConfig)
				...
				if BindConfig.Flag then OrionLibraryrary.Flags[BindConfig.Flag] = Bind end
				return Bind
			end
			...
			function ElementFunction:AddColorpicker(ColorpickerConfig)
				...
				if ColorpickerConfig.Flag then OrionLibraryrary.Flags[ColorpickerConfig.Flag] = Colorpicker end
				return Colorpicker
			end
		...
		function ElementFunction:AddSection(SectionConfig)
			...
			local SectionFrame = SetChildren(SetProps(MakeElement"TFrame", { Size = UDim2.new(1, 0, 0, 26), Parent = Container }), {
				AddThemeObject(SetProps(MakeElement("Label", SectionConfig.Name, 14), { Size = UDim2.new(1, -12, 0, 16), Position = UDim2.new(0, 0, 0, 3), Font = Enum.Font.GothamSemibold }), "TextDark"),
...
function OrionLibraryrary:Destroy() Orion:Destroy() end

return OrionLibraryrary
